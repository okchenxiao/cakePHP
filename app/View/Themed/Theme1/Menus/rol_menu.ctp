...
<SCRIPT type="text/javascript">
    ...
    function getCheckedNodes(data) {
        ...
        if (nodes.length < 1 || data < 1 || isNaN(data) == true) {
            ...
            //执行写入数据库
            $.post("<?php echo $this->Html->url('/Roles/setRol_Menu') ?>", {"key": data,"value": idstring}, function (e) {
                if (e == 1) {
                    alert("赋予菜单操作成功");
                } else {
                    alert("操作失败，请重试。");
    ...
    function settree2(roleId) {
        ...
        if (roleId >= 1) {
            var zNodestr1 = '';
            var authid = '';
            $.post("<?php echo $this->Html->url('/Roles/getRolid_Menulist') ?>", {"key": roleId}, function (e) {
                zNodestr1 = "[";
                var arr1 = e['arr1'];
                var arr2 = e['arr2'];
                for (var i = arr2.length - 1; i >= 0; i--) {
                    zNodestr1 += "{id:" + arr2[i]['id'] + ", pId:" + arr2[i]['pid'] + ", name:'" + arr2[i]['name'];
                    if (arr2[i]['pid'] == 0) {
            ...
            var zNodestr2;
            $.post("<?php echo $this->Html->url('/Menus/getRol_Menulist') ?>", {"key": 1}, function (e) {
                zNodestr2 = "[";
                for (var i = e.length - 1; i >= 0; i--) {
                    zNodestr2 += "{id:" + e[i]['Menu']['id'] + ", pId:" + e[i]['Menu']['pid'] + ", name:'" + e[i]['Menu']['name'];
                    if (e[i]['Menu']['pid'] == 0) {
...
<div class="row-fluid">
    ...
    <div class="span8">
        <div class="portlet box grey">
            <div class="portlet-title">
                <div class="caption"><i class="icon-cogs"></i>菜单赋予</div>
                <div class="tools">
                    <a href="javascript:;" class="collapse"></a>
                    <a href="javascript:;" class="reload"></a>
                </div>
